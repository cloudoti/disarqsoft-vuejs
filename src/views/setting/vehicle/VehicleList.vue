<template>
  <BreadCrumb>Configuración / Vehículos</BreadCrumb>

  <div class="flex items-end justify-end mt-2">
    <router-link
      :to="{name: ERouteType.NEW_VEHICLE_NAME}"
        ...
        <table
          ...
          >
            ...
            <td class="px-6 py-2 text-xs whitespace-nowrap flex items-center justify-center">
              <router-link :to="{name: ERouteType.EDIT_VEHICLE_NAME, params: { id: row.id }}">
...
const paginationUser = ref<Vehicle[]>();
...
const search = async (dataSearch: PaginatedSearch) => {
  loading.value = true;
  paginationUser.value = await VehiclesAPI.List();
